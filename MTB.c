...
{
    clrscr();
    print_char('-',120, 1, LIGHTCYAN);
    print_title_in_middle("Movie Ticket Booking App", 2, MAGENTA);
    print_char('-', 120, 3, LIGHTCYAN);
    print_title("Enter \"0\" to cancle input", 56,4, LIGHTCYAN);
    print_char('-', 120, 6, YELLOW);
    print_char('-', 120, 22, YELLOW);
}
...
void add_questions()
{
    ...
    if(fp==NULL)
    {
        ...
        "Who is your favorite sports columnist?", "Which company would you like to work for in the future?",
        "What is the registration number of your first vehicle?", "What is the name of your best friend?",
        "Who is your favorite author?", "What is your favorite food?", "What is your favorite hobby?",
...
user * get_credentials(char *usertype)
{
    ...
    if(strcmp(usertype,"master_admin")==0)
        print_title_in_middle("Enter Master admin User Id & Password",10,LIGHTCYAN);
    static user u;
...
void input_password(user *u, int column, int row)
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                print_char(' ',80,25,BLACK);
                print_title_in_middle("Password cannot be blank", 25,BLACK);
                getch();
...
char *get_question_id()
{
    ...
    fclose(fp);
    printf("%s ", question_id);//revise basics of C properly
   return question_id;
}
...
void create_user(char *usertype)
{
    ...
    char temp[100];
    char *p;
    if(strcmp(usertype,"admin")==0){ // iThis will true when user type is user then move to their else if part
        ...
        if(up!=NULL){   // if this if will not true then we have to move their else part user pressing zero
            up=get_user_details("admin");
            if(up==NULL){  // this if will run if user login but at remainig data time it will press zerp
                print_char(' ',80,25,BLACK);
            ...
            //set fp to end
            fseek(fp,0,SEEK_END);  //File pointer at end , 0 is end position, SEEK_END is starting point
            fwrite(up,sizeof(user),1,fp); //Record which we recived that write into file, data present in up with help of fwrite (write record of admin in file)
            print_basic_screen(); //clear all previou and print basic screen
            print_title_in_middle("Admin Registered Successfully!",10,YELLOW);
            strcpy(temp,"User Name:"); //copy string User Name into temp
            strcat(temp,up->username); //adding username ravi which is in up with temp
            print_title_in_middle(temp,13,YELLOW); //This will Print User Name Ravi
            strcpy(temp,"User Id");  //User id string copy to temp
            strcat(temp,up->userid);  // addind iuser ad with up->user id
            print_title_in_middle(temp,15,YELLOW);  // it will print user id
            strcpy(temp,"Mobile Number");  // it will copy mobile number into temp
            strcat(temp,up->mobile_number); // it will add with user mobile no
            print_title_in_middle(temp,17,YELLOW); //it will print to screen
            getch();  // screen pause so user can comfortably see the output
            //write the rec
            //show->id, name, mob
        }
        else{
    ...
    {
        // here we have to know what is the id of new coming user if it is first the id is 1 otherwise he has one more id than pervious user id so we have to read record from file for reading record we have to set our file pointer to end of file
        fseek(fp,-(long long signed int )sizeof(user),SEEK_END);
        fread(&usr,sizeof(usr),1,fp); // these two line will give the last record, after reading record from file we have to store it into usr
        // if this is registering first as an user then file has last record of admin
        // else it is not 1st user then file has last record of any user
        if(strcmp(usr.usertype,"admin")==0) // it will check user is of which type, if it is true that mean new user is 1st user
        {
            sprintf(temp,"USER-%d",1);  // it will add 1 with USER ; USER-1
        }
       else {  // it will true if it is not admin
            p=strchr(usr.userid,'-');  //Finding address of - in user id which is in the usr
            int last_id=atoi(p+1);  //atoi function will accept input as string and return integer
            ++last_id;
            sprintf(temp,"USER-%d",last_id);
        }
        up=get_user_details("user"); // up has null (exit(0)) or structure address
        if(up==NULL)
        {
            print_char(' ',80,25,BLACK);
        ...
        strcpy(up->userid,temp);  //up has all thing except user id , id has temp variable now here we copy here
        strcpy(up->usertype,"user");
        fseek(fp,0,SEEK_END); // it will shift file pointer to last of your file
        fwrite(up,sizeof(user),1,fp); // write that at last
        print_basic_screen(); //clear all previou and print basic screen
...
int enter_choice(user *ur)
{
    print_basic_screen();
    char temp[100];
    /*Below three line can be also written as
    strcmp(temp,ur->usertype);
    strcat(temp, ":");
    strcat(temp, ur->username);  */
...
void add_theater()
{
    ...
    if(ftell(fp)==0)
    {
        //sprintf(th.theater_id,"%s","THEATER-1");
        strcpy(th.theater_id,"THEATER-1");
    }
    else
    {
        fseek(fp,-(long long signed int )sizeof(th),SEEK_END);
        fread(&th,sizeof(th),1,fp);  // This will move our pointer to begning of last record
        char *p=strchr(th.theater_id,'-'); // This will print the address of -
        int last_id=atoi(p+1); //It will change your string to int
        ++last_id; // increase by 1
        sprintf(th.theater_id,"THEATER-%d",last_id);  // it will append id with theater
    }
    th.is_booked=0;
    int valid=0;
    do{
...
void show_theater()
{
    ...
    fseek(fp, 0, SEEK_END);
    if(ftell(fp)==0) // It will give zero when file has no record
    {
        print_char(' ', 80,25, BLACK);
...
char * get_theater_id(int status) // status 0-> delete, status 1-> movie add
{
    print_basic_screen();
    FILE *fp=fopen("C:\\Movie Ticket Booking\\Theaters.bin","rb");
    if(fp==NULL)
    ...
    while(fread(&th, sizeof(th),1,fp)==1)
    {
        ...
        {
            if(status==1)
                continue; //Because of this continue our code will reached at while loop and read next record
            print_title(th.theater_id,5,row,YELLOW);
            print_title(th.theater_name,20,row,YELLOW);
...
int delete_movie_by_theater_id(char *th_id)
{
    ...
    FILE *fp2=fopen("C:\\Movie Ticket Booking\\TempMovies.bin","wb");//data write so thats why wb mode
    if(fp2==NULL)
    {
        print_char(' ',79,25,BLACK);
...
int delete_theater()
{
    char *theater_id=get_theater_id(0);  // This pointer p has all theater id
    if(theater_id==NULL)// it has 0, that means user want to cancle
    {
        print_char(' ',80,25,BLACK);
    ...
    theater th;
    while(fread(&th, sizeof(th),1,fp1)==1) // it will read one record of theater id
    {
        if(strcmp(th.theater_id,theater_id)!=0) // compare theater_id local with struct theater id
        {
            fwrite(&th, sizeof(th),1,fp2);  // if not match then write
        }
        else // if match theter_id then
        {
            if(th.is_booked==1)
            {
                int resp=delete_movie_by_theater_id(th.theater_id);  //if delete response 1 otherwise 0
                if(resp==0)
                {
                    fclose(fp1);
                    fclose(fp2);
...
int is_valid_date(struct tm date)
{
 ...
 {
         FILE *fp=fopen("C:\\Movie Ticket Booking\\Theaters.bin","rb+"); // writing also
        if(fp==NULL)
       {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Theaters.bin\n",1,25,RED);
       ...
       // exit(1);
        ...
        while(fread(&th, sizeof(th),1,fp)==1)
        {
            if(strcmp(th.theater_id, th_id)==0)
            {
                th.is_booked=! th.is_booked; // this change will done in varable
                fseek(fp,-4,SEEK_CUR);
 ...
 void add_movie()
 {
    ...
    print_basic_screen();
    FILE *fp=fopen("C:\\Movie Ticket Booking\\Movies.bin","ab+");// read and append add
    if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Movies.bin\n",1,25,RED);
    ...
    movie mov;
    char *p;
    fseek(fp,0,SEEK_END); // file pointer end
    if(ftell(fp)==0) // empty
    {
    ...
    else
    {
        ...
        fread(&mov,sizeof(mov),1,fp);  // This will move our pointer to begning of last record
        p=strchr(mov.movie_id,'-'); // This will print the address of -
        int last_id=atoi(p+1); //It will change your string to int
        ++last_id; // increase by 1
        sprintf(mov.movie_id,"MOVIE-%d",last_id);  // it will append id with theater
    }
    ...
    do
   {
       ...
       if(mov.first_block_price==-1)  // user type string instead of price
       {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Only Digits Alloewd ",25,RED);
        getch();
   ...
   do
   {
       ...
       if(mov.second_block_price==-1)  // user type string instead of price
       {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Only Digits Allowed ",25,RED);
        getch();
   ...
   do
   {
       ...
       if(mov.third_block_price==-1)  // user type string instead of price
       {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Only Digits Alloewd ",25,RED);
        getch();
   ...
   {
       mov.show_date=start; // it will initilized date by today
 ...
 void show_movie()
 {
    ...
    print_title("First Block Price",40,row,BLUE);
    print_title("Second Block Price",55,row,BLUE);
    print_title("Third Block Price",60,row,BLUE);
    rewind(fp);
    movie mov;
    row+=2;
      ...
      FILE *fp=fopen("C:\\Movie Ticket Booking\\users.bin","rb+");
        ...
        else if(choice=='Y' || choice=='y')
        {
            ...
            do
            {
               ...
               {
                   print_char(' ',79,25,BLACK);
            print_title_in_middle("Mobile Number Already Taken!. Try Agian",25,RED);
            getch();
 ...
 void changed_password(user *u)
 {
     print_basic_screen();
      FILE *fp=fopen("C:\\Movie Ticket Booking\\users.bin","rb+");
        ...
        {
            print_char(' ',79,25,BLACK);
            print_title_in_middle("Cancelling Change the Password",25,RED);
            getch();
        }
        else if(choice=='Y' || choice=='y')
        {
            ...
            if(strcmp(us.password,"0")==0)
            {
                fclose(fp);
                print_char(' ',79,25,BLACK);
                print_title_in_middle("Cancelling Changed Password",25,RED);
                getch();
                return;
            }
                        ...
                        while(fread(&us,sizeof(us),1,fp)==1)
            {
                ...
                print_char(' ',79,25,BLACK);
                print_title_in_middle("Password Changed Successfully",25,GREEN);
                getch();
 ...
 {
     print_basic_screen();
     FILE *fp=fopen("C:\\Movie Ticket Booking\\Movies.bin","rb");// read and append add
     if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Movies.bin\n",1,25,RED);
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title_in_middle("Movie Detail Not Present ",25,RED);
        fclose(fp);
        getch();
        return NULL;
    }
 ...
 {
     print_basic_screen();
     FILE *fp=fopen("C:\\Movie Ticket Booking\\SeatNumbers.bin","wb+");// read and append add
     if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file SeatNumbers\n",1,25,RED);
        perror("Reason:");
        getch();
        return NULL;
    }
    ...
    while(fread(&mov, sizeof(mov),1,fp)==1)
        {
            if((strcmp(mov.movie_id,mov_id)==0) && (compare_date(mov.show_date,date)==0))
                {
                  fseek(fp,-(long long unsigned int)sizeof(mov),SEEK_CUR);
 ...
 void booking(user *u)
 {
   ...
   FILE *fp=fopen("C:\\Movie Ticket Booking\\Bookings.bin","ab+");// read and append add
     if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Bookings.bin\n",1,25,RED);
...
void show_booking_of_user(user *u)
{
    ...
    int row=8;
    print_title("Ticket ID",1,row,YELLOW);
    print_title("Theater ID",15,row,YELLOW);
    print_title("Movie ID",30,row,YELLOW);
    print_title("Seat No.",45,row,YELLOW);
    print_title("Show Date",65,row,YELLOW);
    rewind(fp);
    ticket tk;
    row+=2;
    char prev_mov_id[100]={""};
    ...
    while(fread(&tk, sizeof(tk),1,fp)==1)
    {
        if(strcmp(tk.movie_id,prev_mov_id)!=0)
        {
            //display rec
            strcpy(temp_str,tk.ticket_id);
            print_title(temp_str,1,row,YELLOW);

             strcpy(temp_str,tk.theater_id);
            print_title(temp_str,15,row,YELLOW);

            strcpy(temp_str,tk.movie_id);
            print_title(temp_str,30,row,YELLOW);

            sprintf(temp_str,"%d",tk.seat_number);
            print_title(temp_str,45,row,YELLOW);

            sprintf(temp_str,"%02d/%02d/%04d",tk.show_date.tm_mday,tk.show_date.tm_mon,tk.show_date.tm_year);
            print_title(temp_str,65,row,YELLOW);
            row++;
        }
    }
    fclose(fp);
 ...
 ticket * get_ticket_id(user *u)
 {
    ...
    {
         print_char(' ',79,25,BLACK);
        print_title("No Theater Id Present",1,25,RED);
        fclose(fp);
        getch();
        return NULL;
    }
    ...
    print_title("Movie ID",30,row,YELLOW);
    print_title("Show Date",45,row,YELLOW);
    print_title("Seat Number",65,row,YELLOW);
    rewind(fp);
    ticket tk;
    row+=2;
    char prev_mov_id[100]={""};
